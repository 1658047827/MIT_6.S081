...
binit(void)
{
  ...
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){  // wire in sequence
...
bget(uint dev, uint blockno)
{
  ...
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    if(b->dev == dev && b->blockno == blockno){
      b->refcnt++;
      release(&bcache.lock);
      acquiresleep(&b->lock);  // block until lock is acquired
      return b;
    }
  }
  ...
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    if(b->refcnt == 0) {  // refcnt == 0, means free
      b->dev = dev;
      b->blockno = blockno;
      b->valid = 0;
      b->refcnt = 1;
